...
if ($hassiteconfig) {
    ...
    $settings = new admin_settingpage(
        'local_earlyalert_settings',
        get_string('pluginsettings', 'local_earlyalert')
    );
    $ADMIN->add(
        'earlyalert',
        $settings
    ...
    $settings->add(new admin_setting_heading(
            'earlyalert_heading',
            get_string('pluginname', 'local_earlyalert'),
            ''
        )
    );
    // Add a setting to capture streams for Markham campus
    $settings->add(new admin_setting_configtextarea(
            'earlyalert_markham_streams',
            get_string('markham_streams', 'local_earlyalert'),
            '',
            "MPR\nMAH\nMNO
        ")
    );
    ...
    $settings->add(new admin_setting_configcheckbox(
            'earlyalert_showgrades',
            get_string('showgrades', 'local_earlyalert'),
            '',
            0
        )
    );
    ...
    $settings->add(new admin_setting_heading('earlyalert_db_heading', get_string('readonly_database', 'local_earlyalert'), get_string('readonly_database_description', 'local_earlyalert')));
    $settings->add(new admin_setting_configtext('earlyalert_dbuser', get_string('dbuser', 'local_earlyalert'), '', ''));
    $settings->add(new admin_setting_configpasswordunmask('earlyalert_dbpassword', get_string('dbpassword', 'local_earlyalert'), '', ''));
