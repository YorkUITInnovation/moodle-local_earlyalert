define("local_earlyalert/filter_students_grade", ["exports", "core/ajax", "core/str", "core/notification"], function (_exports, _ajax, _str, _notification) {
    "use strict";
...
const loadStrings = async() => {
    ...
    const values = await Promise.all(keys.map(key => _str.get_string(key, 'local_earlyalert').catch(() => key)));
...
const loadStudents = () => {
    ...
    _ajax.call([{methodname: 'local_earlyalert_get_course_students_page', args: {
...
const previewStudent = studentid => {
    ...
    _ajax.call([{methodname: 'local_earlyalert_get_student_preview_template', args: {
        courseid: STATE.courseId,
        teacher_user_id: STATE.teacherUserId,
        studentid,
        alert_type: STATE.alertType,
...
const exportSelectedStudents = () => {
    ...
    if (!selectedIds.length) {
        _notification.alert('', STRINGS.no_students_found || 'No students selected');
        return;
    }
...
const sendAlerts = () => {
    ...
    _notification.confirm(
        ...
        () => {
            _ajax.call([{methodname: 'local_earlyalert_report_log_insert', args: {
                ...
                _notification.alert('', STRINGS.alert_sent_successfully || 'Alert sent successfully');
            }).catch(_notification.exception);
        }
    );
};
